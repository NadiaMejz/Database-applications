...
//? oznacza to może być decimal albo może być null

        decimal? maxSalary = (from emp in emps
            select emp.Sal).Max();
    ...
    {
        var emps = Database.GetEmps();
        decimal? minSalary = (from emp in emps
            where emp.DeptNo == 30
            select emp.Sal).Min();
    ...
    public void ShouldReturnDistinctJobTitles()
    {
        ...
        var jobs = (from emp in emps
            select emp.Job).Distinct().ToList();
    ...
    public void AllEmployeesShouldEarnMoreThan500()
    {
        ...
        var result = (from emp in emps
            select emp).All(emp => emp.Sal > 500);
    ...
    public void ShouldFindAnyWithCommissionOver400()
    {
        ...
        var result = (from emp in emps
            select emp).Any(emp => emp.Comm > 400);
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = (from emp in emps
            select new { emp.EName, Total = emp.Sal + (emp.Comm ?? 0) });
...
// SQL: SELECT E.EName, D.DName, S.Grade FROM Emp E JOIN Dept D ON E.DeptNo = D.DeptNo JOIN Salgrade S ON E.Sal BETWEEN S.Losal AND S.Hisal;
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = (from emp in emps
            ...
            where emp.DeptNo == dept.DeptNo && emp.Sal >= grade.Losal && emp.Sal <= grade.Hisal
            select new { emp.EName , dept.DName, grade.Grade }).ToList();
